...
public class Main : MonoBehaviour {
	...
	void Start () {
		...
		AppDomain.CurrentDomain.UnhandledException += delegate(object sender, UnhandledExceptionEventArgs e) { print(e.ExceptionObject); shutdown(); };
		Thread.GetDomain().UnhandledException += delegate(object sender, UnhandledExceptionEventArgs e) { print(e.ExceptionObject); shutdown(); };
	...
	private void StartEngine()
	{
		...
		foreach (TorrentManager manager in torrents)
		{
			...
			manager.PieceHashed += delegate(object o, PieceHashedEventArgs e) {
				lock (listener)
					print(string.Format("Piece Hashed: {0} - {1}", e.PieceIndex, e.HashPassed ? "Pass" : "Fail"));
			};
			
			// Every time the state changes (Stopped -> Seeding -> Downloading -> Hashing) this is fired
			manager.TorrentStateChanged += delegate (object o, TorrentStateChangedEventArgs e) {
				lock (listener)
					print("OldState: " + e.OldState.ToString() + " NewState: " + e.NewState.ToString());
			};
			...
			foreach (TrackerTier tier in manager.TrackerManager)
			{
				foreach (MonoTorrent.Client.Tracker.Tracker t in tier.Trackers)
				{
					t.AnnounceComplete += delegate(object sender, AnnounceResponseEventArgs e) {
						print(string.Format("{0}: {1}", e.Successful, e.Tracker.ToString()));
					};
				}
			}
...
//				listener.ExportTo(Console.Out);
	...
	{
		lock (listener)
			print(string.Format("Found {0} new peers and {1} existing peers", e.NewPeers, e.ExistingPeers ));//throw new Exception("The method or operation is not implemented.");
	}
	...
	private static void shutdown()
	{
		...
		for (int i = 0; i < torrents.Count; i++)
		{
			...
			{
				print(string.Format("{0} is {1}", torrents[i].Torrent.Name, torrents[i].State));
				Thread.Sleep(250);
			}
		...
		foreach (TraceListener lst in System.Diagnostics.Debug.Listeners)
		{
			lst.Flush();
			lst.Close();
		}
